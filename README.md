...
```git log --all --graph --decorate``` — красивая история коммитов;
...
```git stash list``` — посмотреть что в тайнике

```git stash pop <номер>``` — достать из тайника с номером <номер>, файлы будут как будто после изменения, unstagging


```git branch```   — список веток. Звезщдочкой отмечается текущая ветка

```git branch <название ветки>``` — создать новую ветску с названием <название ветки>

```git branch -D <название ветки> ``` — удалить ветку (коммиты не удаляются)

```git checkout <название ветки>``` — переключиться на  ветску с названием <название ветки>. Если в текущей ветке есть изменения переключаться не будет

```git merge <название ветки>``` — вмержить ветку в текущую ветку. Все коммиты из той ветки добавляются в текущую

```git clone <ссылка на репозиторий>```  — склонировать репозиторий из интернета по ссылке <ссылка на репозиторий>


```git remote```  — пишет текущий удаленный репозиторий (origin - если репозиторий клонировался)

```git remote add <название репозитория> <ссылка на репозиторий>```  — добавить удаленный репозиторий. Он будет создан с названием <название репозитория>. Ссылка может быть как в интернете, так и локально в другой папке на компе

```git push <имя удаленного репозитория> <имя локальной ветки>```  — запушить ветку  <имя локальной ветки> в удаленный репозиторий с именем <имя удаленного репозитория>

```git push <имя удаленного репозитория> <имя локальной ветки>:<имя удаленной ветки>```  — запушить ветку  <имя локальной ветки> в удаленный репозиторий в ветку с именем <имя удаленной ветки>

```git pull <имя удаленного репозитория>```  — забрать изменения из удаленного репозитория

### Откат коммитов

```git reset```  — откатить коммиты

```  --soft <хэш>``` — возвращает к состоянию ```git add``` когда изменения добавлены, но не закомичены 

```  --mixed <хэш>``` (оно же просто reset)  — возвращает изменения в состояние ```unstagged```

```  --hard <хэш>``` — вообще удаляет все изменения, файлы приходят к виду как было раньше

```git revert <хэш>``` — откатить с коммитом. Создастся новый коммит, содержащий обратные преобразования относительно указанного коммита и добавится к истории текущей ветки. 

### Как зачистить фейл если случайно закомитили больше чем нужно

```git reset --hard HEAD~4``` — откатить ветку назад на 4 коммита

```git push --force``` — запушить в удаленный реп, где коммитов больше. По идее они должны бесследно исчезнуть (для этого force)

### Как замержить часть коммитов (но не все) из своей ветки

``` git merge <хэш комммита из ветки dev> ``` — Нужно сначала в своей ветке dev скопировать хэш нужного коммита, затем перейти в master и выполнить эту команду. Закоммитится все, что ниже этого коммита включительно в master, в dev остаются только коммиты выше этого
